var confTab = new Tab();
var addTabNumber = 1;
var save_url = './saveFile/newsave.php';
var setDomain = '';
...
function saveSolutionLoad() {
	...
	ajaxProcess('./saveFile/_save/solution.php', 'mode=load', sucFunc, errorFunc);
}
...
var patchLoad = function () {
	...
	param = "";
	targetUrl = './patch/_fileCheck.php';
	var sucFunc = function(data) {
		$('#verSelect').html('');
		if (data.result) {
...
$(document).ready(function(){
	...
	$('#solutionInfoAdd').on('click', function(){
		...
		ajaxProcess('./saveFile/_save/solution.php', 'mode=save&name=' + solutionName + '&code=' + $('#solutionCode').val(), sucFunc, errorFunc);
	});
	...
	$('#verSetting').on('click', function () {
		...
		if (setDomain) {
			if (selectFile) {
				param = "setDomain=" + setDomain + "&selectVersion=" + urlencode(selectFile);
				targetUrl = './patch/_fileInstall.php';
				...
				alertLayer('로컬 저장 파일을 선택해 주시기 바랍니다.');
			}
		}
		else {
