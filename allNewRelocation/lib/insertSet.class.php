	...
	public function querySet($arrayDataValue, $roopCount) {
		...
		if ($this->multiQueryFl == 'n') {
			foreach ($arrayDataValue as $fieldName => $dataValue) {
				if ((($fieldName == 'writerPw' || $fieldName == 'memPw') && (substr($arrayDataValue[$fieldName], 0, 10) == "password('" || substr($arrayDataValue[$fieldName], 0, 5) == "md5('")) || $arrayDataValue[$fieldName] == 'now()' || $arrayDataValue[$fieldName] == 'NULL') {
