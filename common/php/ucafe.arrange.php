<?php
	...
	foreach ($arrayCafeCSVList as $key => $arrangeInfo) {
		...
		preg_match('/' . $pregText . '/', iconv('EUC-KR', 'UTF-8', $arrayBeforeFileName[$arrangeCount]), $result);
		if (!empty($result)) {
			...
			$arrayLogText[] = "[arrange before data size] : " . number_format(filesize($oldDataBasePath . iconv('UTF-8', 'EUC-KR', $arrayBeforeFileName[$arrangeCount])) / 1048576, 2) . ' MB';
