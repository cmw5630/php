...
class DeprecatedPlateCardPrice
{
  ...
  public function setPriceGrade(&$_plateCard, $_playerCareerHistory)
  {
    /**
    ...
    $_plateCard->plate_c = $plateCWithLeagueTier;

    $_plateCard->grade = $this->getGrade($_plateCard->plate_c, $plateCardSeasonId);
  ...
  protected function getGameD(array $_playerCareerHistory)
  {
    ...
    $only_entry = 0;
    foreach ($_playerCareerHistory as $idx => $career) {
      $total_sub_on += $career['substitute_in'];
      $game_started += $career['appearances'] - $career['substitute_in'];
      $only_entry += $career['subs_on_bench'] - $career['substitute_in'];
    }
  ...
  public function update()
  {
    PlateCard::currentSeason()->isPriceSet(false)->get()->map(function ($plateCard) {
      ...
      $this->setPriceGrade($plateCard, $playerCareerHistory);
      $plateCard->save();
      // <== 
    });
  }
