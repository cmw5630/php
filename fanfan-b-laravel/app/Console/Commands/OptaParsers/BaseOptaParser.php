...
abstract class BaseOptaParser
{
  ...
  protected function optaRequestUrl(array $paths, $_saltParams): array
  {
    return array_map(function ($path) use ($_saltParams) {
      return $this->config['basic']['host'] . '/soccerdata/' . $this->feedType . '/' . $this->config['basic']['outletKey'] . $path . $_saltParams;
    }, $paths);
  }

  protected function optaRequest($ids = [], $_saltParams = '')
  {
    // generate path
    ...
    try {
      ...
      report(new OTPRequestException( // 에러 정의 새롭게 !필요
        null,
        [
  ...
  protected function correctColumnsAndDatetimeType($_row)
  {
    ...
    if (isset($_row['injury_start_date'])) {
      if (!Str::startsWith($_row['injury_start_date'], '20')) {
        $_row['injury_start_date'] = Str::substrReplace($_row['injury_start_date'], '20', 0, 2);
      }
    }

    if (isset($_row['injury_end_date'])) {
      if (Str::startsWith($_row['injury_end_date'], '20')) {
        $_row['injury_end_date'] = Str::substrReplace($_row['injury_end_date'], '20', 0, 2);
      }
    }
        ...
        foreach ($specifiedAttrs[$specifiedKey] as $value) {
          ...
          try {
            if (!empty($specifiedValue['conditions'])) {
              ...
              if (in_array($model, [EventGoal::class, EventCard::class])) {
