...
return new class extends Migration
{
  public function up()
  {
    Schema::create('daily_stat_timelines', function (Blueprint $table) {
      $table->id();
      $table->foreignUuid('schedule_id')->constrained(Schedule::getModel()->getTable(), 'id');
      $table->foreignUuid('season_id')->constrained(Season::getModel()->getTable(), 'id');
      $table->foreignUuid('team_id')->constrained(Team::getModel()->getTable(), 'id');
      $table->char('team_side', 4)->comment('away, home');
      $table->unsignedTinyInteger('period_id')->nullable()->comment('Period Id');
      $table->unsignedSmallInteger('schedule_minute')->default(0)->comment('opta period 경기 시간');
      $table->unsignedSmallInteger('schedule_total_minute')->default(0)->comment('전후반 누적 경기 시간');
      $table->enum('status', ScheduleStatus::getValues());
      $table->decimal('momentum_value', 8, 5)->default(0)->comment('팀별 모멘텀 포인트');
      $table->decimal('momentum_y', 8, 5)->default(0)->comment('실제 표시되는 모멘텀 y값');
  ...
  {
    Schema::dropIfExists('daily_stat_timelines');
  }
};
